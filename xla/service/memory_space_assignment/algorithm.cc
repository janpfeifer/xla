...
void MsaAlgorithm::FindAliases(
  ...
  absl::flat_hash_map<const HloInstruction*, std::vector<size_t>>
      value_indices_by_defining_inst;
  for (size_t idx = 0; idx < allocation_values->size(); ++idx) {
    value_indices_by_defining_inst[allocation_values->at(idx)
                                       .defining_instruction()]
        .push_back(idx);
  }
  auto maybe_add_alias_with_instruction = [&](const HloInstruction* instruction,
                                              AllocationValue::Use* use) {
    auto aliased_values_it = value_indices_by_defining_inst.find(instruction);
    if (aliased_values_it != value_indices_by_defining_inst.end()) {
      for (const size_t aliased_value_idx : aliased_values_it->second) {
        VLOG(3) << "Adding aliasing for use " << use->hlo_use.ToString()
                << " to "
                << allocation_values->at(aliased_value_idx).ToShortString();
        use->aliases.push_back(
            allocation_values->at(aliased_value_idx).defining_position());
      }
    }
  };
  ...
  for (AllocationValue& value : *allocation_values) {
    for (AllocationValue::Use& use : value.uses()) {
      ...
      // Special case for kWhile: the root of the body computation must alias
      // as well.
...
std::vector<const HloValue*> MsaAlgorithm::GetIntervalsClosure(
  ...
  // TODO(mehrdadk): use the sorted_buffer_intervals order instead
...
absl::StatusOr<HeapSimulator::Result<HloValue>> MsaAlgorithm::Finish() {
  ...
  for (auto& closure_entrance_interval : sorted_buffer_intervals) {
    ...
    auto intervals_closure = GetIntervalsClosure(closure_entrance_interval);
    if (VLOG_IS_ON(3)) {

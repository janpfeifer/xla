...
DotInfo InnerDotInfo(const DotInfo& dot_info) {
  DCHECK(IsBatchDot(dot_info)) << "DotInfo must be a batch dot";

  DotInfo inner_dot;

  inner_dot.lhs_shape = ShapeUtil::DeleteDimensions(
      dot_info.dim_nums.lhs_batch_dimensions(), dot_info.lhs_shape);
  inner_dot.rhs_shape = ShapeUtil::DeleteDimensions(
      dot_info.dim_nums.rhs_batch_dimensions(), dot_info.rhs_shape);
  inner_dot.result_shape = ShapeUtil::DeleteDimensions(
      dot_info.dim_nums.lhs_batch_dimensions(), dot_info.result_shape);

  inner_dot.dim_nums = dot_info.dim_nums;
  inner_dot.dim_nums.clear_lhs_batch_dimensions();
  inner_dot.dim_nums.clear_rhs_batch_dimensions();

  DCHECK_EQ(dot_info.dim_nums.lhs_contracting_dimensions_size(), 1);
  DCHECK_EQ(dot_info.dim_nums.rhs_contracting_dimensions_size(), 1);

  int64_t num_batch_dims = dot_info.dim_nums.lhs_batch_dimensions_size();
